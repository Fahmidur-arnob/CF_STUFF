...
void Kakashi_of_the_Sharingan(){
    ...
    for(ll i = 1; i < n; i++){
        previous[i] = arr[i] + previous[i - 1];
    }
    while(m--){
        ll okL, okR, okK; cin >> okL >> okR >> okK;

        ll kiss = previous[okR - 1] - (previous[okL - 1] - arr[okL - 1]);

        ll notKiss = answer - kiss + (okR - okL + 1) * okK;
